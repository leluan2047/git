...
function initializeDefaultVariables($database_driver)
{
    ...
    switch ($database_driver) {
        case 'pdo_pgsql':
            ...
            putenv('DBUSER='.(getenv('DBUSER') ? getenv('DBUSER') : 'root'));
        ...
        case 'pdo_mysql':
            ...
            putenv('DBSERVER='.(getenv('DBSERVER') ? getenv('DBSERVER') : 'localhost'));
            putenv('DBNAME='.(getenv('DBNAME') ? getenv('DBNAME') : 'eccube3_0_13'));
            putenv('DBUSER='.(getenv('DBUSER') ? getenv('DBUSER') : 'root'));
            putenv('DBPORT='.(getenv('DBPORT') ? getenv('DBPORT') : '3306'));
            putenv('DBPASS='.(getenv('DBPASS') ? getenv('DBPASS') : ''));
            if (getenv('TRAVIS')) {
                putenv('DBPASS=');
                putenv('ROOTPASS=');
            } else {
                putenv('DBPASS='.(getenv('DBPASS') ? getenv('DBPASS') : ''));
                putenv('ROOTPASS='.(getenv('ROOTPASS') ? getenv('ROOTPASS') : (getenv('DBPASS') ? getenv('DBPASS') : '')));
            }
            break;
        default:
        case 'pdo_sqlite':
